...
class CreateVehicleServiceComponent extends Component
{
    ...
    {
        $createVehicleServiceResult = $this->executeAction($this->createVehicleServiceAction, [
            'name' => $this->name,
            'status' => $this->status,
        ], true);

        if ($createVehicleServiceResult->isSuccess()) {
            session()->flash('success', __('panel.vehicle_service_created_successfully'));
            return redirect()->route('v1.panel.vehicle-services.index');
        }
