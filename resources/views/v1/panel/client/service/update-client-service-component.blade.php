...
@section('description', __('panel.update_client_service'))
...
<x-containers.card-container>
    <form wire:submit.prevent="updateClientService">
            ...
            <x-forms.form-field label="{{ __('panel.service') }}*" for="service_id" :error="$errors->first('service_id')">
                ...
                >
                    ...
                    @foreach($services as $service)
                        <flux:select.option value="{{ $service->id }}" selected="{{ $service_id == $service->id ? 'selected' : '' }}">{{ $service->name }}</flux:select.option>
                    @endforeach
                </flux:select>
            </x-forms.form-field>
